...
Read more: https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts`), Promise.reject(new Error(o))
      } let { serviceWorkerVersion: i, serviceWorkerUrl: r = u(`flutter_service_worker.js?v=${i}`), timeoutMillis: e = 4e3 } = t, n = r; this._ttPolicy != null && (n = this._ttPolicy.createScriptURL(n)); let a = navigator.serviceWorker.register(n).then(o => this._getNewServiceWorker(o, i)).then(this._waitForServiceWorkerActivation); return T(a, e, "prepareServiceWorker")
...
logoImage.src = 'icons/vertical_logo.png'; // Path to your image
logoImage.alt = 'Logo';
logoImage.style.width = '100px';
logoImage.style.height = 'auto';
loadingContainer.appendChild(logoImage);
...
loadingContainer.textContent = "Loading Entrypoint...";
_flutter.loader.load({
  serviceWorkerSettings: {
    serviceWorkerVersion: "1012487071"
  },
  onEntrypointLoaded: async function (engineInitializer) {
    loadingContainer.textContent = "Initializing engine...";
    const appRunner = await engineInitializer.initializeEngine();

    loadingContainer.textContent = "Running app...";
    await appRunner.runApp();

    // Remove loading container once the app is ready
